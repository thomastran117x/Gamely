...
def make_service(
    ...
    AuthService, FakeRepository, FakeRedis, FakeEmailPublisher, FakeTokenService
]:
...
async def test_signup_normalizes_email_and_rejects_duplicate() -> None:
    existing = make_user()
    service, repository, _, _, _ = make_service(FakeRepository([existing]))
...
async def test_signup_rolls_back_code_and_user_transaction_on_publish_failure() -> None:
    ...
    service, repository, redis, _, _ = make_service(email=publisher)
...
async def test_code_cooldown_prevents_duplicate_delivery() -> None:
    service, _, _, publisher, _ = make_service()
...
async def test_unknown_email_and_delivery_failure_are_silent_for_enumeration_routes() -> (
    ...
    unknown_service, _, _, unknown_publisher, _ = make_service()
    ...
    known_service, _, redis, _, _ = make_service(
...
async def test_code_is_deleted_after_five_failed_attempts() -> None:
    service, _, redis, _, _ = make_service()
...
async def test_reset_password_rejects_invalid_code_without_mutation() -> None:
    user = make_user()
    service, repository, _, _, tokens = make_service(FakeRepository([user]))
...
async def test_reset_password_hashes_password_and_revokes_all_sessions() -> None:
    user = make_user()
    service, repository, redis, _, tokens = make_service(FakeRepository([user]))
