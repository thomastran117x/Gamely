...
async def declare_topology(channel: aio_pika.abc.AbstractChannel) -> aio_pika.abc.AbstractExchange:
    ...
    primary = await channel.declare_queue(QUEUE, durable=True, arguments={"x-dead-letter-exchange": EXCHANGE, "x-dead-letter-routing-key": "dead"})
    await primary.bind(exchange, "send")
    for delay in (30, 60, 120, 240):
        queue = await channel.declare_queue(f"auth.email.retry.{delay}", durable=True, arguments={"x-message-ttl": delay * 1000, "x-dead-letter-exchange": EXCHANGE, "x-dead-letter-routing-key": "send"})
        await queue.bind(exchange, f"retry.{delay}")
    return exchange


class EmailPublisher:
    def __init__(self, connection: aio_pika.abc.AbstractRobustConnection | None) -> None:
    ...
    async def publish(self, job: EmailJob) -> None:
        ...
        try:
            ...
            await exchange.publish(aio_pika.Message(job.to_bytes(), delivery_mode=aio_pika.DeliveryMode.PERSISTENT, content_type="application/json"), routing_key="send")
        finally:
            await channel.close()
