...
class AuthController:
    ...
    async def login(
        self, body: CredentialsRequest, response: Response
    ) -> TokenResponse:
    ...
    async def change_password(
        self, body: ChangePasswordRequest, request: Request, response: Response
    ) -> dict[str, str]:
        await self._auth.change_password(
            self._current_user(request), body.current_password, body.new_password
        )
    ...
    async def oauth_login(
        self, provider: str, body: OAuthTokenRequest, response: Response
    ) -> TokenResponse:
        _, access, refresh = await self._oauth.login(
            provider, body.id_token, body.nonce
        )
        self._set_refresh(response, refresh)
        return TokenResponse(access_token=access)
    ...
    def _set_refresh(self, response: Response, value: str) -> None:
        response.set_cookie(
            key=self._settings.auth_refresh_cookie_name,
            value=value,
            httponly=True,
            secure=self._settings.auth_refresh_cookie_secure,
            samesite=self._settings.auth_refresh_cookie_samesite,
            domain=self._settings.auth_refresh_cookie_domain,
            path=self._settings.auth_refresh_cookie_path,
            max_age=self._settings.auth_refresh_token_days * 86400,
        )
...
async def verify_email(
    body: VerifyEmailRequest, controller: AuthController = Depends(get_controller)
) -> dict[str, str]:
...
async def reset_password(
    body: ResetPasswordRequest, controller: AuthController = Depends(get_controller)
) -> dict[str, str]:
...
@router.get("/me")
async def me(
    request: Request, controller: AuthController = Depends(get_controller)
) -> dict[str, object]:
    return await controller.me(request)


@router.post("/oauth/{provider}/challenge")
async def oauth_challenge(
    provider: str, controller: AuthController = Depends(get_controller)
) -> dict[str, str]:
